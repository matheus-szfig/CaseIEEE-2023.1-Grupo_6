...
export async function cadastrateUserService(
  email,
  senha,
  nome,
  senhaConfirmacao,
  ...
  try {
    ...
    cargos.forEach((c) => {
      const verificador =
        verificaOpcao(c["cargo"], cargoOpcoes) &&
        verificaOpcao(c["equipe"], equipeOpcoes);
    ...
    const idUser = await database("usuario")
      .select("id")
      .where({ email: email })
      .first();
    for (let i = 0; i < cargos.length; i++) {
      ...
      let cargoUser = await database("cargo")
        .select("id")
        .where({ nome: cargos[i].cargo })
        .first();
      let equipesUser = await database("equipe")
        .select("id")
        .where({ nome: cargos[i].equipe })
        .first();
