...
export default function GraphResult({votacao}) {
  ...
  async function getVotacaoData () {
    ...
    const vUsers = votos?.data?.usuarios?.sort((a, b) => {
      return parseInt(b.qnt) - parseInt(a.qnt)
    }).splice(0, 3).map((u, i) => ({...u, lugar:i}));;
    ...
    const vEquipes = votos?.data?.equipes?.sort((a, b) => {
      return parseInt(b.qnt) - parseInt(a.qnt)
    }).splice(0, 3).map((e, i) => ({...e, lugar:i}));
    ...
    <div className="flex flex-col md:flex-row justify-between h-full gap-5">
      <div className="flex flex-col w-full justify-between md:pb-[10rem] md:ps-3">
        ...
        <div className="flex w-full gap-1">
          {
            votosEquipes.map((e) => {
              console.log('equipe:', e.id_equipe, getColor(e.lugar), getSize(e.lugar))
              return <VelaGraph nome={e.nome} cor={getColor(e.lugar)} tamanho={getSize(e.lugar)} qnt={e.qnt} key={'e'+e.id_equipe}/>
            })
          }
      ...
      <div className="flex flex-col w-full justify-between md:pb-[10rem] md:pe-3">
        ...
        <div className="flex w-full gap-1">
          {
            votosUsuarios.map((u) => {
              console.log('usuario:', u.id_usuario, getColor(u.lugar), getSize(u.lugar))
              return <VelaGraph nome={u.nome.split(' ')[0]} cor={getColor(u.lugar)} tamanho={getSize(u.lugar)} qnt={u.qnt} key={'u'+u.id_usuario}/>
            })
          }
        </div>
