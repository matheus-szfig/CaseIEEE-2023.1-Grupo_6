...
export default function EquipeAccordion({id, nome, members}) {
  ...
  async function FetchMembers () {
    const filtered_members = members?.filter(v => v.cargos.filter(c => c.aprovado).map(c => parseInt(c.id_equipe)).includes(parseInt(id)))
      .map(u => ({
        id: u.id,
        nome: u.nome,
        cargos: u.cargos.filter(c => parseInt(c.id_equipe) === parseInt(id)).map(c => c.cargo).toString().replace(/\,/, ', ')
      }))
  ...
  return (
    <>
      <div className={`w-full h-50 mb-1 bg-primary rounded flex flex-col text-white`}>
        <div className="flex justify-between">
          <div className="flex flex-col justify-center w-full ps-4 py-2 hover:cursor-pointer"
            onClick={(e) => {
              e.preventDefault();
              setOpen(!open);
            }}
          ...
          <div className="flex">
            ...
            <div className="flex flex-col justify-center" >
              <input onClick={CheckEquipe} checked={equipeVotos.includes(id)} className="hover:cursor-pointer ms-3 me-4 w-5 h-5 bg-gray-100 text-yellow-400 border-gray-300 rounded-full focus:ring-yellow-400 focus:color-yellow-400 focus:ring-2" type="checkbox"/>
            </div>
          </div>
        </div>
      </div>
